...
async function main() {
  ...
  const readiness = await probePrivateDocumentLifecycle({
    routeUrl: `${baseUrl}/api/private-document-promote`,
    token: await accessToken(),
    projectId: requiredEnv("AR006_PROJECT_ID"),
    documentId: randomUUID(),
  });
  console.log(
    `AR-006 ADR 0012 route preflight reached the lifecycle DO after ${readiness.attempts} bounded attempt(s); safe status sequence [${readiness.statuses.join(",")}].`,
  );
}
main().catch((error) => {
