...
comment on function public.update_venue_core(uuid, uuid, bigint, text, text, text, text) is
  'Ordinary venue identity update with venues.write checked before and after row locking, expected-revision optimistic locking and database-level input normalization.';
comment on function public.transition_venue_status(uuid, uuid, text, text, bigint) is
  'Pre-contractual venue lifecycle command with venues.write checked before and after row locking, expected-revision optimistic locking, rejection validation and retained history. Protected commitment/terminal states require dedicated future commands.';
