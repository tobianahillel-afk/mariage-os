...
- `WP29C-AR-005` — **MAJOR** — incomplete interrupted-staging/abandon cleanup plus abandon↔promotion TOCTOU/orphan divergence.
- `WP29C-AR-006` — **MAJOR** — exact-25-MB Workers Free CPU feasibility not evidenced by local workerd success.
- `WP29C-AR-007` — **MAJOR** — release/deployment/secret operations not reconciled with the new Pages Function trust boundary.
