...
9. `WP-1.9` — Storage/Realtime isolation foundation and security-matrix closure — **IN_PROGRESS**.
...
- Review-failed/blocked packets: **none currently; WP-1.9 is actively remediating `WP19-AR-001`**
...
- WP-1.9 historical pre-review evidence: run `33996464530` on `a0e719462b2ea41908e6438a72cac75f11edcf36`, 5/5 SUCCESS including clean-checkout `npm run verify`; DB **14 files / 277 tests / PASS**. Fresh Pass B found `WP19-AR-001`, so this evidence is historical for acceptance.
- Current branch: **`lot-1/identity-project-foundation`**
- Next permitted action: **repair `WP19-AR-001` only by adding direct transactional DELETE allow/deny evidence using `storage.allow_delete_query=true`; then fresh exact-head evidence and fresh Pass B**.
...
- WP-1.1 through WP-1.8 are accepted foundations; WP-1.9 is in bounded remediation as the final implementation packet before Lot reconciliation/integration/acceptance.

## Current blockers / forward maintenance

Open finding under active remediation:

- `WP19-AR-001` **MAJOR**: DELETE policy shape is correct, but direct behavioral allow/deny evidence was missing. Remediation uses the exact transaction-local flag Supabase Storage uses to permit metadata deletion, then executes DELETE under the existing synthetic roles while RLS remains active.
...
Current: WP-1.9 / IN_PROGRESS / A-REMEDIATION
Open: WP19-AR-001
Next: bounded DELETE-RLS evidence repair -> fresh exact-head verification -> fresh Pass B
