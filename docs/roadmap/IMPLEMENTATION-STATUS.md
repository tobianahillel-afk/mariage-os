...
2. The replacement account token passed isolated CI verification and an Observability telemetry query at HTTP `200`; the two wrong-scope older Observability tokens were removed. The 2026-09-24 architecture decision permits narrowly scoped Pages/Worker deployment-token recovery, then an isolated `[AR006-PREFLIGHT]`, then one fresh exact-size evidence campaign. The old 2026-09-17 Worker logs exceeded Workers Free retention.
...
Current permitted action: under the 2026-09-24 decision, replace the expired Pages deploy token and expiring Worker deploy token with one-year, minimal account tokens, run `[AR006-PREFLIGHT]`, then one fresh exact-size evidence campaign only if preflight passes; AR-006 remains open.
...
Lots 3–12: NOT_STARTED
Next permitted action: rotate isolated Pages/Worker deployment tokens under the 2026-09-24 decision, pass `[AR006-PREFLIGHT]`, then run one exact-size provider-evidence campaign; no old-log requery
```
