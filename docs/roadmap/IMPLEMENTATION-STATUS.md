...
1. Record preflight retry `eca478937fad40632dc378f0408c1c8ec4bd5c8c` / CI `36049934080` / job `107805560963` as failed-contained at the immediate deny-smoke after every provider configuration/deployment check before it passed.
2. Preserve the successful provider state from that retry; do not authorize another Worker deploy or Pages PATCH merely to retest readiness.
3. Remediate the smoke narrowly: only HTTP 404 may be retried for a bounded readiness window, and success still requires exact HTTP 405 + JSON generic-unavailable semantics. Unexpected non-404 responses fail immediately; persistent 404 fails closed.
4. Add a marker-gated `[AR006-DO-PREFLIGHT-READONLY]` continuation pinned to the exact existing `eca478...` Pages deployment. It may only GET provider configuration/deployment metadata, re-run synthetic authority checks, run the bounded deny smoke and execute the non-mutating random-unreserved-document route probe. It must not deploy, PATCH, reserve, upload, promote or finalize.
5. Run ordinary exact-head CI + clean-checkout verification and fresh remediation review before triggering that single continuation.
6. If the continuation is green, implement and adversarially review the new **two-surface CPU evaluator** before enabling any `[AR006-DO-EVIDENCE]` path. The exact-size job remains hard-disabled until that review is clean.
...
Current packet: WP-2.9C — IN_PROGRESS — ADR 0012 EXISTING-DEPLOYMENT PREFLIGHT COMPLETION REMEDIATION/REVIEW; AR-006 remains OPEN
...
Current permitted action: exact-head-green smoke-propagation remediation + fresh review, then one non-mutating `[AR006-DO-PREFLIGHT-READONLY]` continuation pinned to the existing `eca478...` deployment. No further deploy/PATCH and no exact-size provider campaign are authorized by this state.
