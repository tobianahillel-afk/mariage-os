...
4. Current action is repository-only remediation: bounded 404/503-only lifecycle-route readiness with final exact 409/generic-unavailable semantics, shared by standalone route probe and marker preflight, plus a sanitized pre-mutation failure receipt.
5. After remediation passes exact-head CI + clean checkout, perform a fresh targeted adversarial review.
6. Only after that review may one read-only recheck of the already-created exact `6bdf...` preview test readiness without Worker deploy, Pages PATCH, reservation, upload, promotion or finalization. A second exact-size campaign is **not** automatic.
7. Do not substitute wall time, dashboard aggregate, Paid entitlement or a lower file limit. AR-006 remains OPEN until valid provider evidence exists.
