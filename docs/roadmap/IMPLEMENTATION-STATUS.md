...
2. Provider-harness review is clean at implementation head `5b0cc21bc8bb051edea5ec51841709b307ba5365` / CI `36073556382`, including clean checkout. The review closed partial-receipt, exact-Worker-SHA attribution and stale-runbook findings.
3. Next action is repository-only: replace the hard-disabled exact-size job with a strict push + `lot-2/venues-core` + `[AR006-DO-EVIDENCE]` marker predicate and update its contract test. The enablement commit itself must **not** contain the marker.
4. Require ordinary exact-head CI + clean checkout on that gate-enablement commit while the provider job remains skipped. Only after that green state may one same-tree no-content marker commit authorize one bounded exact-size provider campaign.
5. The campaign must run ten exact `25,000,000`-byte synthetic flows. Pages must be `stateless` with numeric CPU `<=10 ms`; each lifecycle invocation must be `durableObject` with numeric CPU `<=30,000 ms`, exact Worker version, distinct DO identity, `outcome=ok` and no CPU-limit outcome. Missing/ambiguous/truncated telemetry fails closed.
6. Do not substitute wall time, dashboard aggregate, Paid entitlement or a lower file limit. AR-006 remains OPEN until that provider evidence is valid.
...
Current packet: WP-2.9C — IN_PROGRESS / ADR 0012 PROVIDER HARNESS REVIEW GREEN / MARKER-GATE ENABLEMENT NEXT; AR-006 remains OPEN
...
Current permitted action: repository-only enablement of the strict [AR006-DO-EVIDENCE] marker gate, followed by ordinary exact-head CI/clean checkout with no marker. No provider campaign is authorized until that enablement head is green.
