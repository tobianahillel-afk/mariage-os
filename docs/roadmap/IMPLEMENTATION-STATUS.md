...
1. Preserve the successful provider state from preflight retry `eca478937fad40632dc378f0408c1c8ec4bd5c8c`; do not perform another Worker deploy or Pages PATCH merely to retest readiness.
2. The bounded 404-only readiness remediation is exact-head green and freshly reviewed with no BLOCKING/MAJOR finding in continuation scope.
3. After this review/status commit itself is exact-head green, trigger exactly one marker-gated `[AR006-DO-PREFLIGHT-READONLY]` continuation pinned to the existing `eca478...` deployment.
4. That continuation may only GET provider configuration/deployment metadata, re-run synthetic authority checks, run the bounded deny smoke and execute the non-mutating random-unreserved-document route probe. It must not deploy, PATCH, reserve, upload, promote or finalize.
5. If the continuation is green, implement and adversarially review the new **two-surface CPU evaluator** before enabling any `[AR006-DO-EVIDENCE]` path. The exact-size job remains hard-disabled until that review is clean.
...
Current packet: WP-2.9C — IN_PROGRESS — ADR 0012 EXISTING-DEPLOYMENT READ-ONLY CONTINUATION GATE; AR-006 remains OPEN
...
Current permitted action: after the review/status reconciliation commit is exact-head green, trigger one non-mutating `[AR006-DO-PREFLIGHT-READONLY]` continuation pinned to the existing `eca478...` deployment. No further deploy/PATCH and no exact-size provider campaign are authorized by this state.
