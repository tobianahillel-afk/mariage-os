...
import promotionCoreSource from "../../../functions/api/private-document-promotion-core.ts?raw";
...
describe("WP-2.9C ADR-0009 bodyless Pages promotion boundary", () => {
  it("rejects framed bodies without consuming untrusted request bytes", () => {
    ...
    expect(promotionCoreSource).not.toContain("request.arrayBuffer()");
    expect(promotionSource).not.toContain("getReader()");
    expect(promotionCoreSource).not.toContain("getReader()");
    ...
    expect(promotionCoreSource).toContain("requestHasBodyFrame");
