...
import ingressWorker from "../../../workers/private-document-ingress/src/worker.ts?raw";
...
describe("ADR 0013 Workers Static Assets ingress RED", () => {
  it("has a dedicated public Static Assets ingress Worker", () => {
    expect(ingressConfig).toContain('"name": "mariage-os-ar006-ingress"');
    expect(ingressConfig).toContain('"directory": "../../dist"');
    expect(ingressConfig).toContain('"run_worker_first": ["/api/*"]');
    expect(ingressConfig).toContain('"PRIVATE_DOCUMENT_LIFECYCLE"');
    expect(ingressConfig).not.toContain("PRIVATE_DOCUMENT_ADMIN_KEY");
    expect(ingressWorker).toContain('"worker-ingress"');
  });
