...
it("maps explicit venue fact freshness RPC arguments exactly", async () => {
...
] as const)("maps freshness provider error %# to %s", async (error, code) => {
...
it("maps freshness transport rejection without leaking details", async () => {
  const adapter = new SupabaseVenueFactFreshnessAdapter({
...
it("rejects malformed successful freshness responses", async () => {
  const adapter = new SupabaseVenueFactFreshnessAdapter(
