...
it("rejects invalid dates, statuses, instants, sources and notes", () => {
  expect(
    normalizeVenueAvailability(draft({ eventDate: "2027-02-29" })),
  ).toEqual({ ok: false, error: "invalid_event_date" });
  expect(normalizeVenueAvailability(draft({ status: "maybe" }))).toEqual({
    ok: false,
    error: "invalid_status",
  });
  expect(normalizeVenueAvailability(draft({ observedAt: "tomorrow" }))).toEqual(
    { ok: false, error: "invalid_observed_at" },
  );
  expect(normalizeVenueAvailability(draft({ optionExpiresAt: "bad" }))).toEqual(
    { ok: false, error: "invalid_option_expires_at" },
  );
  expect(
    normalizeVenueAvailability(
      draft({ optionExpiresAt: "2026-09-10T16:00:00Z" }),
    ),
  ).toEqual({ ok: false, error: "invalid_option_expires_at" });
  expect(normalizeVenueAvailability(draft({ sourceId: "not-a-uuid" }))).toEqual(
    { ok: false, error: "invalid_source_id" },
  );
  expect(normalizeVenueAvailability(draft({ notes: 12 }))).toEqual({
    ok: false,
    error: "invalid_notes",
  });
  expect(
    normalizeVenueAvailability(draft({ notes: "x".repeat(5_001) })),
...
it("compares canonical replay payloads independently of audit fields", () => {
  const first = record();
  expect(venueAvailabilityPayloadEquals(first, record({ revision: 7 }))).toBe(
    true,
  );
  expect(
    venueAvailabilityPayloadEquals(first, record({ notes: "changed" })),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(
      first,
      record({ projectId: "55555555-5555-4555-8555-555555555555" }),
    ),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(
      first,
      record({ venueId: "55555555-5555-4555-8555-555555555555" }),
    ),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(first, record({ dateOptionId: sourceId })),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(first, record({ eventDate: "2027-06-13" })),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(first, record({ status: "unknown" })),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(
      first,
      record({ optionExpiresAt: "2026-09-09T10:00:00.000Z" }),
    ),
  ).toBe(false);
  expect(
    venueAvailabilityPayloadEquals(
      first,
      record({ observedAt: "2026-09-09T10:00:00.000Z" }),
    ),
  ).toBe(false);
  expect(venueAvailabilityPayloadEquals(first, record({ sourceId }))).toBe(
    false,
  );
});
...
it("selects latest deterministically by observedAt, createdAt, then id", () => {
  ...
  expect(latestVenueAvailability([oldest, laterCreated, idWinner])).toEqual(
    idWinner,
  );
  expect(latestVenueAvailability([])).toBeNull();
  expect(
    latestVenueAvailability(new Array<VenueAvailabilityRecord>(1)),
  ).toBeNull();
});
...
it("derives elapsed option holds as expired without rewriting stored status", () => {
  const held = record({
    status: "option_held",
    optionExpiresAt: "2026-09-09T10:00:00.000Z",
  });
  expect(
    effectiveVenueAvailabilityStatus(held, "2026-09-09T09:59:59.999Z"),
  ).toBe("option_held");
  expect(
    effectiveVenueAvailabilityStatus(held, "2026-09-09T10:00:00.000Z"),
  ).toBe("expired");
  expect(
    effectiveVenueAvailabilityStatus(record(), "2026-09-10T10:00:00.000Z"),
  ).toBe("available");
  expect(
    effectiveVenueAvailabilityStatus(
      record({ status: "option_held", optionExpiresAt: null }),
