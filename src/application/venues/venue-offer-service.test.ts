...
class FakeVenueOfferPort implements VenueOfferPort {
  ...
  readonly createVenueOffer = vi.fn(
    async (): Promise<VenueOfferAggregateRecord> => ({
      offer: offerRecord(),
...
it("creates an atomic normalized venue offer aggregate", async () => {
...
it("rejects invalid venue offer and component creation inputs", async () => {
  const port = new FakeVenueOfferPort();
  await expect(
...
it("supports offer creation without components and maps persistence failures", async () => {
  const port = new FakeVenueOfferPort();
  const noComponents = {
    offerId,
    projectId,
...
it("updates only valid expected-revision venue offer draft terms", async () => {
  const port = new FakeVenueOfferPort();
  const input = {
    projectId,
    venueId,
...
it("validates venue offer lifecycle transition identity, revision and target", async () => {
  const port = new FakeVenueOfferPort();
  const input = {
    projectId,
    venueId,
