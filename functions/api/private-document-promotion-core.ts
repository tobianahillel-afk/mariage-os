...
function requestAuthority(
  ...
  if (requireSameOrigin && !requestOriginAllowed(request)) {
...
async function handlePromotion(
  ...
  const authority = requestAuthority(request, env, requireSameOrigin);
...
export async function handleTrustedPromotion(
  request: Request,
  env: PrivateDocumentPagesEnvironment,
  requireSameOrigin = true,
): Promise<Response> {
  try {
    return await handlePromotion(request, env, requireSameOrigin);
  } catch {
