...
6. **Implementation/release governance:** `docs/engineering/*`, lot/checkpoint/release contracts.
...
The phrase `freeze candidate` in the historical header of `docs/domain/PHYSICAL-SCHEMA-V1.md` is **not a competing status or permission signal**. That document is a normative implementation schema reference; its explicit addenda control their narrower corrected scopes, while current implementation permission is controlled only by `IMPLEMENTATION-STATUS.md` + `FINAL-DESIGN-REVIEW.md`. A future cleanup may simplify that historical label, but no agent may interpret it as permission to choose an alternative schema.
...
- read the accepted V1 scope/spec/Feature Ledger as historical baseline;
...
- map cloud, IndexedDB, sync, import, backup, settings and UI migration impacts;
- preserve historical Requirement/Feature/Acceptance traceability;
- use the V1→V2 controls in `docs/reviews/ABSOLUTE-300-CONTROL-CHECKLIST.md`.
...
- release/deployment credentials exist only in protected CI environments and never in untrusted PRs.
...
Use applicable layers from `docs/quality/TESTING-STRATEGY.md`, `docs/quality/QUALITY-GATES.md` and `docs/engineering/DEFINITION-OF-DONE.md`, including as relevant:
...
- post-deployment release health evidence.

In-scope coverage targets do not replace meaningful assertions.

The exhaustive cross-phase control set is `docs/reviews/ABSOLUTE-300-CONTROL-CHECKLIST.md`. Controls become checked only when their phase-specific evidence exists; documentation does not masquerade as runtime verification.
...
- an obsolete client cannot safely coexist with the target backend and no update-required path exists.
